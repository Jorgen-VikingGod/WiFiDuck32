...
void begin() {
   ...
   */
  cli.addCommand("help", [](cmd* c) { print(cli.toString()); });
   ...
   */
  Command cmdSet{cli.addCommand("set", [](cmd* c) {
    Command cmd{c};
   ...
   */
  Command cmdRename{cli.addCommand("rename", [](cmd* c) {
    Command cmd{c};
   ...
   */
  Command cmdWrite{cli.addCommand("write", [](cmd* c) {
    Command cmd{c};
  ...
  cli.addCommand("read", [](cmd* c) {
    ...
    */
      String buffer = spiffs::streamRead();
  ...
  /**
   * \brief Create select_profile command
   *
   * Prints profile path by given profile index
   *
   * \param * profile index
   */
  cli.addSingleArgCmd("select_profile", [](cmd* c) {
    Command cmd{c};
    Argument arg{cmd.getArg(0)};

    profile::setProfile(arg.getValue().toInt());
    String res = profile::currentProfile();
    print(res);
  });

  /**
   * \brief Create next_profile command
   *
   * select next profile
   */
  cli.addCommand("next_profile", [](cmd* c) {
    profile::next();
    String res = profile::currentProfile();
    print(res);
  });
  ...
  /**
   * \brief Create prev_profile command
   *
   * select prev profile
   */
  cli.addCommand("prev_profile", [](cmd* c) {
    profile::prev();
    String res = profile::currentProfile();
    print(res);
  });
  ...
  /**
   * \brief Create save_profile command
   *
   * save current profile config
   */
  cli.addCommand("save_profile", [](cmd* c) {
    profile::saveConfiguration();
    print("> saved profile config");
  });

  /**
   * \brief Create display command
   *
   * show current profile on display
   */
  cli.addCommand("display", [](cmd* c) {
    profile::showMenu();
    print("> show display");
  });
}
...
void parse(const char* input, PrintFunction printfunc, bool echo) {
  ...
  if (spiffs::streaming() && (strcmp(input, "close\n") != 0) && (strcmp(input, "read\n") != 0)) {
