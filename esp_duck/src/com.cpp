...
void i2c_transmit(char b) { Wire.write(b); }
...
void i2c_request() {
  ...
  react_on_status = status.wait == 0 || status.repeat > 0 || ((prev_wait & 1) ^ (status.wait & 1));
...
void serial_start_transmission() { debug("Transmitting '"); }
...
void serial_transmit(char b) { SERIAL_PORT.write(b); }
...
unsigned int send(char str) { return send(&str, 1); }

unsigned int send(const char* str) { return send(str, strlen(str)); }
...
void onDone(com_callback c) { callback_done = c; }

void onRepeat(com_callback c) { callback_repeat = c; }

void onError(com_callback c) { callback_error = c; }

bool connected() { return connection; }

int getVersion() { return status.version; }
}  // namespace com